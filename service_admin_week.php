...
if(isset($_POST['Navigate']) && ($_POST['Navigate'] == 'submitEdit')){
	...
	if(!$result){
		...
		$result = mysql_query($sql);
		//echo "<meta http-equiv=\"refresh\" content=\"0;URL='http://localhost/service_admin_week.php'\">";
	}
}
...
$result = mysql_query($sql);
	if(!$result){
		...
		while($r = mysql_fetch_array($result)){
			...
			echo "<meta http-equiv=\"refresh\" content=\"0;URL='http://localhost/service_leader_dashboard.php?d=".$detail_id."&z=".$i.$additional_info."&o=".$occurrence_id."'\">";
		}
	}
}
...
echo "<h3>Run new week setup for {$date1} to {$date2} <a href=\"http://localhost/service_admin_week_setup.php\">here</a>.</h3>";
echo "<table border=0 class=\"displayListingTable\">";
echo "<tr class=\"displayListing\"><td>date</td><td></td><td>name</td><td>start</td><td>end</td><td>current</td><td>max</td><td>hours</td><td></td></tr>";
$sql = "SELECT d.detail_id, d.event_id, d.DOW,
		o.start, o.end, o.length, o.max, e.P_Id,
...
echo "<h4><a href=\"http://localhost/service_admin.php?run=KDj83jJ$\">modify defaults</a></h4>";
