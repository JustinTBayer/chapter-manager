<?php
function refresh(){
	echo("<meta http-equiv=\"REFRESH\" content=\"0;url=http://localhost/service_admin.php\">");
}
		...
		echo "<tr class=\"displayListing\"><td>day</td><td>event</td><td>name</td><td>start</td><td>end</td><td></td></tr>";
				...
				$PLASize = count($projectLeaderArray);
					...
					for($v=0;$v<=$PLASize;$v++){
						...
						if($theDOW==$DOW){
							echo ("<tr class=\"rowm1\"><td>$DOW</td><td>$name</td><td>$firstname $lastname</td><td>$start</td><td>$end</td><td><a href=\"http://localhost/service_admin.php?remove=1&d=".$detail_id."&u=".$user_id."\">X</a></td></tr>");
						}
				}	
		}
		echo "</table>";
